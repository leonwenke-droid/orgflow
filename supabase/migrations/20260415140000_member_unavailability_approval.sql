-- Approval workflow for rotation unavailability (like task transfers).
-- Only approved rows block rotation; pending/rejected do not.
...
comment on column public.member_unavailability.status is
  'pending until admin/lead approves; only approved affects rotation_preview / rotation_assign.';

create index if not exists idx_member_unavailability_org_status
  on public.member_unavailability (organization_id, status)
  where status = 'pending';

-- ---------------------------------------------------------------------------
-- rotation_preview / rotation_assign: only count approved unavailability
-- ---------------------------------------------------------------------------
...
-- Members must not be able to approve their own requests via direct DB access.
  ...
  or (
    ...
    and exists (
      ...
      where p.auth_user_id = auth.uid()
        and p.organization_id = member_unavailability.organization_id
        and coalesce(p.status, 'active') <> 'disabled'
        and coalesce(p.role::text, '') in ('admin', 'owner', 'lead', 'teamlead', 'super_admin')
    )
  )
);
