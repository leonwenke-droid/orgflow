-- Rotation fairness: when assigning via rotation for a shift, do not reuse the same
-- rotation "round" for another shift on the same day.
--
-- Concretely: treat "already assigned" as "assigned to ANY shift for that org + date",
-- not just "assigned within the current shift".
...
begin
  ...
  for r in
    ...
    where p.organization_id = p_org_id
      and coalesce(p.status, 'active') <> 'disabled'
      and coalesce(p.role::text, '') <> 'viewer'
      -- do not reuse members already assigned on the same day (any shift in org)
      and not exists (
        select 1
        from public.shift_assignments sa
        join public.shifts sh on sh.id = sa.shift_id
