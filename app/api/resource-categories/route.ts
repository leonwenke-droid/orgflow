...
export async function POST(req: Request) {
  try {
    ...
    // Authorize: org admins via RPC (RLS) or super admin. We do the write with service role to keep
    // this endpoint stable even if category RLS changes, but only after auth checks.
    ...
    const rows = categories
      .map((c: any) => ({
        ...
        examples: c.examples != null ? String(c.examples) : null,
        enabled: true
      }))
    ...
    console.error("[resource-categories]", e);
